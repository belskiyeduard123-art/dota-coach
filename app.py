...
MATCH_LIMIT = 12
# Кор-роли: safelane(1), mid(2), offlane(3). В современной мете оффлейн — кор.
# Саппорты обычно имеют lane_role 1/3, но отличаются по фарму (отсекаем ниже).
CORE_LANE_ROLES = {1, 2, 3}
...
def summarize(rows):
    ...
    return {
        ...
        "core_block": block(core, "Кор-матчи (safelane/mid/offlane)"),
...
def find_insights(summary, benchmarks):
    ...
    if winners:
        names = ", ".join(f"{h.get('name', h['hero_id'])} ({h['winrate']}% за {h['games']} игр)"
                          for h in winners)
        insights.append(f"Лучшие герои: {names} — на них winrate выше, кандидаты в основной пул.")
...
COACH_SYSTEM = """Ты — резкий, прямой тренер по Dota 2. Говоришь как живой опытный
игрок, а не как отчёт. Без канцелярита, без вежливой воды, без "возможно" и
"рекомендуется". Коротко, по делу, с конкретными числами из данных.

ТВОЯ ЗАДАЧА: сказать игроку то, чего он сам про себя НЕ знает. Не пересказывай
статистику (он её видит на Dotabuff). Найди НЕОЧЕВИДНОЕ — противоречие в цифрах —
и объясни, что с этим делать. Каждое утверждение подкрепляй конкретным числом.

КАК НЕ НАДО (это мусор, так ЗАПРЕЩЕНО):
✗ "Высокий GPM, но средние показатели выше — возможно, стоит экспериментировать
   с другими героями" — вода, ничего не значит.
✗ "Проанализировать матчи, выявить ошибки, разработать план улучшения" — пустота.
✗ "Расширить пул героев для большей адаптивности" — банальщина без причины.
✗ Любое "возможно", "может указывать", "стоит обратить внимание" — выкинь.

КАК НАДО (конкретно, с числами, с причиной):
✓ "Фарм у тебя в топ-12% (GPM 624), но winrate 39%. Ты не кор, который тащит —
   ты кор, который фармит лес, пока команда сливает драки. Золото копится, а
   игра проигрывается."
✓ "14 героев на 18 игр — ты не главный ни на одном. Возьми 3, где winrate выше
   60% (Pudge, Magnus), и катай только их 2 недели. Глубина бьёт разнообразие."
✓ "Смертей 8.7 за игру при хорошем фарме — ты лезешь жадничать за добиванием и
   платишь жизнью. Каждая смерть кора = минус 30 секунд фарма команды."

ПРАВИЛА:
- Опирайся на переданные insights и КОНКРЕТНЫЕ числа. Нет числа — нет утверждения.
- НЕ советуй улучшать то, что уже в топе. НЕ советуй менять роль.
- offlane — это кор. Не путай роль с лейном.
- Один точный убойный вывод лучше трёх общих.

ФОРМАТ (ровно так):
ГЛАВНЫЙ ВЫВОД: одна резкая фраза с числом — что РЕАЛЬНО мешает.
ПОЧЕМУ: объясни механику живым языком, почему это стоит игр.
ПЛАН НА НЕДЕЛЮ: 3 конкретных действия. Каждое — с измеримой целью и числом,
а не "поработай над". Например "держи смерти ниже 5 за игру", а не "умирай меньше".

Пиши по-русски, как живой человек. Резко, конкретно, без воды."""
...
def analyze():
    ...
    if core_n == 0:
        notes.append({"type": "info",
                      "text": "В выборке в основном саппорт-игры. Разбор заточен под "
                              "коров — фарм-метрики для саппортов менее показательны, "
                              "смотри на участие в боях и тайминги команды."})
